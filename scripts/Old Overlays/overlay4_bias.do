...
label var bias_reduction  "Reduction in Absolute Bias"
...
//Figure 
twoway (contourline bias_reduction  corr_N1_K_out corr_N1_N2_out if bias_reduction!=. ,  ccuts(`lb1' `lb2' `lb3' `lb4' `lb5' `lb6' `lb7' `lb8' 0 0 `ub1' `ub2' `ub3' `ub4' `ub5' `ub6' `ub7' `ub8') ///
...
title("Case 4: g{sub:N{sub:1}}<0 and g{sub:N{sub:2}}<0") ///
...
legend(order(1 "" 2 "" 3 "" 4 "")  size(tiny) )  ///
 saving("$figs/overlay4_bias.gph", replace) 
