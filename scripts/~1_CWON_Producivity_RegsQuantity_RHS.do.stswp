...
*/
merge 1:1 countrycode year using "$processed/pwt100.dta", keep(3)
drop _merge

gen iso3 = countrycode

// merge the euro area version from Eurostat
merge 1:1 year iso3 using "$processed/euro_area_mfp_panel_iso.dta", gen(merge_EU)
drop if merge_EU == 2

// FAO TFP panel
merge 1:1 year countrycode using "$processed/UN_FAO_TFP_panel.dta", gen(merge_UNFAO)
